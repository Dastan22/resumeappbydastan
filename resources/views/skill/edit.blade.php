...
@section('content')
    ...
    <form method="POST" action="{{route('skill.update', $skill)}}">
        ...
        <div class="mb-3">
            ...
            <input name="rating" value="{{$skill->rating}}" type="text" class="form-control @error('rating') is-invalid @enderror" id="rating" aria-describedby="rating">
            @error('rating')
            <span class="invalid-feedback" role="alert">
                {{ $message }}
            </span>
